    ...
    return Scaffold(
      ...
      body: AsyncView(
        ...
        empty: () => const EmptyState(
          ...
          message: 'A unit can\'t be quoted until it has a base rate.',
        ),
        data: (list) {
            ...
            children: [
              for (final kind in RateKind.values)
                if (grouped[kind] != null && grouped[kind]!.isNotEmpty) ...[
                  SectionHeader(title: _kindLabel(kind)),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.of(context).push(
          MaterialPageRoute(builder: (_) => RateRuleFormScreen(unitId: unitId)),
        ),
...
class RateRuleFormScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<RateRuleFormScreen> createState() => _RateRuleFormScreenState();
}
...
class _RateRuleFormScreenState extends ConsumerState<RateRuleFormScreen> {
  ...
  void initState() {
    ...
    _cleaningFee = TextEditingController(
      text: existing?.cleaningFee.toString() ?? '0',
    );
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await ref.read(rateRepositoryProvider).upsert(rule, id: existing?.id);
