...
class _TasksScreenState extends ConsumerState<TasksScreen> {
  ...
  Future<void> _delete(BuildContext context, TaskFilter filter, StaffTask task) async {
    // The buttons pop with the dialog's own context: showDialog puts the
    // dialog on the root navigator, but inside the router's ShellRoute the
    // screen's `context` resolves to the shell navigator, so popping that
    // would remove the Tasks page and leave the dialog up.
