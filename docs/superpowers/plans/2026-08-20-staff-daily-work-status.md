...
-- raises 42501, so no trigger is needed for this path. "Today" is
-- judged against the resort's own local calendar day (India Standard
-- Time), not `current_date` (the database server's own configured
-- timezone) -- a device physically at the resort naturally represents
-- "today" in IST, and this must agree with the check, or a check-in
-- near local midnight is silently rejected.
...
-- A `using (true)` UPDATE policy plus an enforcement trigger -- the
-- pattern the two sibling features use -- was tried first and doesn't
-- work here: Postgres RLS requires SELECT-policy visibility on a row
-- before an UPDATE policy's own USING clause is even consulted, and a
-- same-tier staff peer attempting to check someone else out has none
-- (they're neither the row's owner nor admin) -- so a raw client UPDATE
-- would silently affect zero rows instead of raising an error. This
-- function runs as its own owner (bypassing RLS on its internal
-- queries, same as every other SECURITY DEFINER function in this
-- schema), does its own explicit ownership and already-checked-out
-- check, and raises an immediate, clear error rather than depending on
-- RLS row-visibility to gate the caller.
