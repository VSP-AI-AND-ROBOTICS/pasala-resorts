...
select lives_ok(
  $$insert into public.leave_requests (property_id, id, staff_id, start_date, end_date, reason)
    values ('a0000000-0000-0000-0000-000000000001', '98111111-1111-1111-1111-111111111111',
            '10000000-0000-0000-0000-000000000003','2026-09-10','2026-09-12',
            'Family trip')$$,
  'staff can insert their own pending leave request');

select throws_ok(
  $$insert into public.leave_requests (property_id, staff_id, start_date, end_date)
    values ('a0000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000004','2026-09-15','2026-09-16')$$,
  '42501', null, 'staff cannot insert a leave request for someone else');

select throws_ok(
  $$insert into public.leave_requests (property_id, staff_id, start_date, end_date, status)
    values ('a0000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000003','2026-09-20','2026-09-21',
            'approved')$$,
  '42501', null, 'staff cannot insert a request that is already approved');

select throws_ok(
  $$insert into public.leave_requests (property_id, staff_id, start_date, end_date, decided_by)
    values ('a0000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000003','2026-09-25','2026-09-26',
            '10000000-0000-0000-0000-000000000002')$$,
  '42501', null, 'staff cannot insert with decided_by set on a pending request');

select throws_ok(
  $$insert into public.leave_requests (property_id, staff_id, start_date, end_date, decided_at)
    values ('a0000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000003','2026-10-01','2026-10-02',
            now())$$,
...
select throws_ok(
  $$insert into public.leave_requests (property_id, staff_id, start_date, end_date)
    values ('a0000000-0000-0000-0000-000000000001', '98333333-3333-3333-3333-333333333333','2026-09-10','2026-09-12')$$,
  '42501', null,
...
select throws_ok(
  $$insert into public.leave_requests (property_id, staff_id, start_date, end_date)
    values ('a0000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000003','2026-09-20','2026-09-10')$$,
...
select lives_ok(
  $$insert into public.leave_requests (property_id, id, staff_id, start_date, end_date)
    values ('a0000000-0000-0000-0000-000000000001', '98222222-2222-2222-2222-222222222222',
...
-- RLS (leave_requests_update is admin-only) filters this: no error, no
-- row changed.
select is(pg_temp.rows_affected(
  $$update public.leave_requests set status = 'approved'
    where id = '98111111-1111-1111-1111-111111111111'$$),
  0, 'a staff member cannot approve their own request');
