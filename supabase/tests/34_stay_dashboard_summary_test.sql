...
select is(
  ((public.dashboard_summary()) ->> 'currently_in_house')::int,
  1,
  'currently_in_house counts the one checked_in reservation'
);

select is(
  ((public.dashboard_summary()) ->> 'checked_out_today')::int,
  1,
  'checked_out_today counts the reservation checked out just now'
);

select is(
  ((public.dashboard_summary()) ->> 'checked_in_today')::int,
  0,
...
select ok(
  (public.dashboard_summary()) ? 'today_revenue'
    and (public.dashboard_summary()) ? 'food_sales_today',
